---
sidebar_class_name: hidden
---

:::warning
There is a pending German patent application with the application number 10 2023 125 012.4. In
order to use the Innamark watermarker Software in the form published here, a patent license is
required in addition to the license for the Software. See [`LICENSE`](https://github.com/FraunhoferISST/Innamark/blob/main/LICENSE)
for more information. In case of any questions or uncertainties, please contact us at
innamark@isst.fraunhofer.de.
:::
